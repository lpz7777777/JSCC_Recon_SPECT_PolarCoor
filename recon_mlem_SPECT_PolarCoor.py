...
def mlem_list_mode(t_list, img, s_map, rotmat, rotmat_inv, osem_subset_num, rotate_num, t_divide_num):
...
def mlem_list_mode_moreevents(t_list, img, s_map, rotmat, rotmat_inv, osem_subset_num, rotate_num, t_divide_num, device):
...
def mlem_list_mode_moreemorevents(t_list, img, s_map, rotmat, rotmat_inv, osem_subset_num, rotate_num, t_divide_num, device):
    # list mode mlem algorithm for large size of t
    # cuda stream of loading t from cpu to gpu
    stream_loader = torch.cuda.Stream(device=device)
    t_pinned_list = []
...
def mlem_joint_mode(sysmat_list, proj_list, t_list, img, s_map, rotmat, rotmat_inv, alpha, osem_subset_num, rotate_num, t_divide_num):
    for i in range(osem_subset_num):
        weight = 0 * img
        for j in range(rotate_num):
            for k in range(t_divide_num):
...
def mlem_joint_mode_moreevents(sysmat_list, proj_list, t_list, img, s_map, rotmat, rotmat_inv, alpha, osem_subset_num, rotate_num, t_divide_num, device):
...
def run_recon_mlem(sysmat, rotmat, rotmat_inv, proj, proj_d, t, iter_arg, s_map_arg, alpha, save_path, device):
    ...
    if iter_arg.flag_moreevents == 1:
        ...
        t_list[0][0][0] = t_list[0][0][0].to(device)
    ...
    for i in range(0, iter_arg.osem_subset_num):
        sysmat_list.append(sysmat[cpnum_list[i], :].to(device))
        proj_list.append(proj[cpnum_list[i], :].to(device))
        proj_d_list.append(proj_d[cpnum_list[i], :].to(device))

    rotmat = rotmat.to(device)
    rotmat_inv = rotmat_inv.to(device)
    s_map_arg.s = s_map_arg.s.to(device)
    s_map_arg.d = s_map_arg.d.to(device)
    ...
    if iter_arg.flag_moreevents == 1:
        ...
        for id_iter_jsccd in range(iter_arg.jsccd):
            img_jsccd = mlem_list_mode_moreevents(t_list, img_jsccd, s_map_arg.d, rotmat, rotmat_inv, iter_arg.osem_subset_num, rotate_num, iter_arg.t_divide_num, device)
        ...
        for id_iter_jsccsd in range(iter_arg.jsccsd):
            img_jsccsd = mlem_joint_mode_moreevents(sysmat_list, proj_list, t_list, img_jsccsd, alpha * s_map_arg.s + (2 - alpha) * s_map_arg.d, rotmat, rotmat_inv, alpha, iter_arg.osem_subset_num, rotate_num, iter_arg.t_divide_num, device)
        ...
        id_save = 0
        for id_iter_jsccd in range(iter_arg.jsccd):
            img_jsccd = mlem_list_mode(t_list, img_jsccd, s_map_arg.d, rotmat, rotmat_inv, iter_arg.osem_subset_num, rotate_num, iter_arg.t_divide_num)
        ...
        id_save = 0
        for id_iter_jsccsd in range(iter_arg.jsccsd):
            img_jsccsd = mlem_joint_mode(sysmat_list, proj_list, t_list, img_jsccsd, alpha * s_map_arg.s + (2 - alpha) * s_map_arg.d, rotmat, rotmat_inv, alpha, iter_arg.osem_subset_num, rotate_num, iter_arg.t_divide_num)
            if (id_iter_jsccsd + 1) % iter_arg.save_iter_step == 0:
                img_jsccsd_iter[id_save, :] = torch.squeeze(img_jsccsd).cpu()
                id_save += 1
                print("Iteration ", str(id_iter_jsccsd + 1), " ends, time used:", time.time() - time_start, "s")
